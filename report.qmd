...
onlygaps_hetero <- Agegaps_hetero |>
  filter(age_difference != 0) |>
  mutate(older_partner = if_else(actor_1_age > actor_2_age & 
                                   character_1_gender == "man", 
                                 "Male", "Female"))

onlygaps_hetero$older_partner <- as.factor(onlygaps_hetero$older_partner)    
...
```{r}
#| label: rsquaredhetero
onlygapsheteroR <- linear_reg() |>
set_engine("lm") |>
  fit(release_year ~ age_difference, data = onlygaps_hetero)
```
```{r}
glance(onlygapsheteroR)$adj.r.squared
```
