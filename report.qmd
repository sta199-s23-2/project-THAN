...
Agegaps_hetero <- Agegaps_hetero |>
  mutate(older_partner = if_else(actor_1_age > actor_2_age & 
                                 ...
                                 if_else(actor_1_age == actor_2_age, 
                                         "No Gap", "Female"))) 
...
```{r}
#| label: agegap-summary-hetero
Agegaps_hetero |> 
  group_by(older_partner) |>
  summarize(m = mean(age_difference , na.rm = T))
```

```{r}
#| label: agegap-hetero-clt
est_mu_m <- 11.7
Agegaps_h_m <-Agegaps_hetero |> 
  filter(older_partner == "Male")
est_sigma_m <- sd(Agegaps_h_m$age_difference , na.rm = T) / sqrt(917)
qnorm(c(0.025, 0.975), est_mu_m, est_sigma_m)

est_mu_f <- 5.50
Agegaps_h_f <- Agegaps_hetero |> 
  filter(older_partner == "Female")
est_sigma_f <- sd(Agegaps_h_f$age_difference , na.rm = T) / sqrt(185)
qnorm(c(0.025, 0.975), est_mu_f, est_sigma_f)

```

The scatter plot above shows every movie observation over time where the couple has an age gap of atleast one year, the x-axis plotting the year the film was released and the y-axis plotting the age gap in years. Each point is shaded either pink, if the older partner was female, or blue, if the older partner was male. The graph shows that the older partner in Hollywood movie relationships is disproportionately male, and that in cases where the older partner is female, the age gap tends to be on the smaller side. This is supported by calculations using the Central Limit Theorem where we can say with 95% confidence that the true mean age gap in couples where the older partner was female is between only 4.6 and 6.4 years whereas the true mean age gap in couples where the older partner was male is between 11.2 and 12.2 years. 

In addition, the three best fit lines, purple representing all couples regardless of the gender of the older partner, all show that over time the size of these age gaps has decreased.

```{r}
#| label: age-gap-histo-decades

Agegaps_updated <- Agegaps_hetero |>
...
Agegaps_samesex <- Agegaps_samesex |>
  mutate(older_partner = if_else(actor_1_age > actor_2_age & 
                                   character_1_gender == "man", "Gay",
                                 if_else(actor_1_age == actor_2_age, 
                                         "No Gap", "Lesbian")))
...
Agegaps_samesex |> 
  group_by(older_partner) |>
  summarize(mean = mean(age_difference, na.rm = T))
```

Looking at only the data for same-sex couples, we get a completely different picture. Since 1997, the earliest datapoint for same sex relationships in the dataset, the average age gap between partners has increased. Given there are less observations available for samesex couples, we can not justify using the Central Limit Theorem to create a confidence interval ($12 < 30$, $11 < 30$) so we will use bootstrapping to determine the mean size of the age gap between partners portrayed in same sex relationships.

```{r}
#| label: agegap-samesex-bootstrap

set.seed(12345)

Agegaps_s_m <-Agegaps_samesex |> 
  filter(older_partner == "Gay")
...
Agegaps_s_f <-Agegaps_samesex |> 
  filter(older_partner == "Lesbian")

female_df <- Agegaps_s_f |>
  specify(response = age_difference) |>
  generate(reps = 10000, type = "bootstrap") |>
  calculate(stat = "mean")
...
```{r}
#| label: agegap-hetero-1997

Agegaps_hetero_1997 <- Agegaps_hetero |>
  mutate(older_partner = if_else(actor_1_age > actor_2_age & 
                                   character_1_gender == "man", "Male",
                                 if_else(actor_1_age == actor_2_age, 
                                         "No Gap", "Female"))) |>
...
```{r}
#| label: agegap-summary-hetero-1997
Agegaps_hetero_1997 |> 
  group_by(older_partner) |>
  summarize(m = mean(age_difference , na.rm = T))
```
...
```{r}
#| label: agegap-hetero-clt-1997
est_mu_m_1997 <- 10.7
Agegaps_h_m_1997 <-Agegaps_hetero_1997 |> 
  filter(older_partner == "Male")
est_sigma_m_1997 <- sd(Agegaps_h_m_1997$age_difference , na.rm = T) / sqrt(670)
qnorm(c(0.025, 0.975), est_mu_m_1997, est_sigma_m_1997)

est_mu_f_1997 <- 5.39
Agegaps_h_f_1997 <- Agegaps_hetero_1997 |> 
  filter(older_partner == "Female")
est_sigma_f_1997 <- sd(Agegaps_h_f_1997$age_difference , na.rm = T) / sqrt(157)
qnorm(c(0.025, 0.975), est_mu_f_1997, est_sigma_f_1997)

```
For gay same sex couples, we can say with 95% confidence that the true mean age gap is between 9 and 22.7 years, whereas for lesbian couples the mean gap is between 6.45 and 16.3 years. 
Over the same time frame (1997-2021), for heterosexual couples, we can say with 95% confidence that the true mean age gap was between 10.1 and 11.3 years for couples where the male partner was older and 4.5 to 6.2 years for couples where the female partner was older. 
