...
From the plot we observe that in cases where an age gap is present, the older partner tends to be male. And in cases where the older partner is female, the age difference tends to be smaller than the age difference for couples where the older partner is male.
...
The bar chart above groups movie observations into the era that they were released. The plot shows that as over time the proportion of films containing age gaps of over 15 years decreases.
...
The box plot above visualizes the mean age difference in films by the five directors most represented in *Hollywood Age Gaps*. The plot shows that some directors, like Woody Allen and John Glen, when compared to the mean age difference of heterosexual couples in the dataset ($\mu = 10.4$) tend to produce movies with a larger than average age gap; whereas others, like Martin Scorsese produce movies with around average sized age gaps, and Mike Newell's movies contain below average sized age gaps.
